...
impl VectorStore {
    ...
    pub fn search(&self, query: &str, top_k: usize) -> Result<Vec<SearchResult>> {
        ...
        let results = stmt.query_map(
            params![sanitize_fts_query(query), top_k as i64],
            |row| {
                Ok(SearchResult {
                    file: row.get(0)?,
    ...
    pub fn grep(&self, pattern: &str, top_k: usize) -> Result<Vec<SearchResult>> {
        ...
        let results = stmt.query_map(params![like, top_k as i64], |row| {
            Ok(SearchResult {
                file: row.get(0)?,
                line: row.get::<_, i64>(1)? as usize,
                snippet: row.get::<_, String>(2)?.chars().take(300).collect(),
    ...
    pub fn clear_file(&self, file: &str) -> Result<()> {
        self.conn.execute("DELETE FROM chunks WHERE file = ?1", params![file])?;
        Ok(())
    }
